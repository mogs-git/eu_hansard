...
# Part 2: Text mining a Hansard

```{r}
###### PROBLEMS ENCOUNTERED ##########
# appearances_tib %>% filter(speech_id == 174) 
# For some reason, even in teh original "eu" text file, the formatting removes Lord Pannick's
# name/header when he moves a new clause after Baroness Jones. 
# str_sub(eu, start = 265351, end = 266500)
# This error occurs whenever someone says "After Clause...", so will extend the previous Lords speech
# Remove these parts?
...
This article will explore how text mining can be used to gather data from Parliamentary transcripts (Hansards). This will build on ideas from another article I wrote: if you haven't read the first part, find that here. 

What information do we want to mine a piece of text for? You can imagine that we have a set of documents, an example of this might be a set of news articles about the performance of different companies. Often, articles don't come in a format immediately appropriate for analysis so will first have to be "cleaned" and "standardised", but we will ignore that for now. 

Every text mining analysis seeks to convert features of the text into numerical values. This is just a fancy way of saying: first, we need to count things. Lots of text mining approaches are really just counting things and using different methods to compare those counts. Sometimes we might have to divide one of our counts by another thing we counted to make sure we can compare things fairly. For example, we might have counted the number of adjectives used in two speeches. One speech used 18 adjectives and another used only 10. But it turns out the first speech is 600 words long where the second speech is only 200, so if we account for this we might say the first speech uses 3 adjectives per 100 words, whereas the second uses 5 per 100 words. Who is really using "more adjectives"? You get the picture. It turns out there are lots of fancy ways you can play with your counts that will help you make interesting, reliable and meaningful observations and inferences about a text, and we will discuss several of them during this analysis. 

But what's our goal here? What can we do with this data that you can't get from just reading the text? The first goal is summarisation. We can condense information down and extract the most important parts in a way that can be easily scaled. The second goal is clarification. When you read a debate, you have certain intuitions and feelings about what went on. "Those three spoke about similar things". "He was the most aggressive". We want to quantify these things, and by putting numbers on them make statements in a statistically rigorous, evidence-based way. We can then in an unbiased way search for every pattern in the text, even ones that people might miss after a few readings. The third goal for lots of text mining work is prediction. You have gathered information on what is there, and maybe you can use this again in the future to predict something in another text? This is a bit beyond the scope of this article, but perhaps you can use data you have gathered from one set of speeches to predict the party of a speaker in a new speech.  
...
- Number of pairs/multiples of words
- Number of other grammatical features of the text (punctuation, grammatical category)

Of course, things get a bit more exciting than this. There are two other important things to consider:
- The Hansard has a temporal element, we can order the speeches chronologically
- grammar in context blargh

### Speech lengths

We'll start with the simplest detail relating to the words in our Hansard- how many are used in total. This data is just a count of the number of words in each speech. 
...
# For all speeches made by each Lord
...
at_nostop %>%
  ...
  #ggrepel::geom_text_repel(data=bind_rows(gather(at_ns_sum, stat, val), tibble(stat="total", val=NA)), label = stat)
...
As we can see, even these raw numbers of words tell a story. But now we will move onto the text itself. 

Let's get some boring definitions out of the way first. Each text is composed of words. People who research [text mining](https://en.wikipedia.org/wiki/Text_mining) and apply this in [natural language processing](https://en.wikipedia.org/wiki/Natural_language_processing) (NLP) refer to each body of text as a document. Think different newspaper articles, different books by an author. We could treat all of these as documents. A corpus is the word used to refer to a collection of documents. Now let's think about scale. There are lots of Hansards about Brexit. So we could (and later will) make a corpus of Hansard documents. But here, I have chosen to focus on what can be gleaned from a single debate. For our purposes, we will treat *speeches* as each document, and the corpus will be the collection of speeches in the Hansard. 


------


What words are most important in each speech? How can we rank words in terms of importance? Simple questions. Difficult to answer. We'll go through some examples to understand why defining which words are important to a speech is difficult, and how to get around these problems. But first, here is *one way* researchers calculate the importance of a word: TF-IDF.
...
After counting the frequency of each word in a speech, we can remove **stop words**. This is literally an arbitrary list of "common" words which we have decided not to include in the analysis and remove. It's not flashy, but it's useful for what we're doing here
...
What do we actually mean by *important*? Imagine you have a set of newspapers articles about education. The word "school" and "teacher" probably appears in every document (with varying frequency), so we could penalise these words allowing other words that distinguish different speeches to be found. Now, we could just remove these common words (treat them as "stop words"). But there is a better solution.  

#### Solution

When we have our **term frequency**, we can divide this by the **"Inverse document frequency"** for that word. How do we calculate this? Say you have 10 documents. You're word appears in 8 of them (it's a pretty common word). The inverse document frequency is equal to `log(10/8)`. Let's compare how this changes if we make our word rarer amongst the documents.

```{r}
idftib <- tibble(
  `Word in X documents` = 8,
  `Total number of documents` = 10^(1:6),
  `Inverse document frequency` = log(`Total number of documents`/`Word in X documents`),
...
The unlogged is shown for comparison. We just log the value so that this "IDF multiplier" stays within a reasonable range, the IDF doesn't increase too rapidly for small numbers of documents or large numbers. 

That aside, as documents containing our word become *rarer*, their IDF score *increases*. This is the important thing. Think about what this means. 

TF-IDF ("Word importance") = Term frequency * Inverse Document Frequency

More frequent terms get a higher score (good). Words that appear in a smaller fraction of documents get a higher score (also good). So if a word is unique to a speech and is used a often, it gets a high score, because it defines that speech and not others. Low scoring words are either not used a lot in any speeches, or appear in almost every document- meaning they can't be used to accurately define any of them.
...
```{r, echo = FALSE, message = FALSE, warnings = FALSE, fig.width=6, fig.height=6}
...
Looks messy but let's talk it through. Generally, TF-IDF seems to have pulled out words that are important in these speeches, ranked somewhere in the top 10. There are also words that sometimes are important and that don't make sense without further context. Other words seem to not have the "right" rank if we were to do it ourselves intuitively. But in general, we can see that speech 148 is about womens rights, 109 about euratom and nuclear safety and 228 about the role of devolved administrations. 1 and 106 are less clear but seem to be about fair rights for eu nationals and uk citizens. Speech 152 seems a bit unclear and no one word has a particularly high tf-idf. 

------

As a complete and inconsequential aside before we move on, here are the TF-IDF scores from every speech in rank order from highest to lowest.
...
TF-IDF has several cool uses. Some more complicated variations of this simple metric allow search engines to find documents most relevant to a searched keyword. We are going to use it as a way to summarise texts. It will be like pulling important words from a "bag of words"- which is our text. The meaning from the context which the words were used in is inevitably lost, but it's a useful heuristic and the way TF-IDF is calculated is transparent (not a black box). 
...
In this article, we have explored how simply counting the number of words in speeches can reveal things about a Hansard. We talked in detail about how we can use these counts to create a useful scoring system (TF-IDF) for finding important words in documents. We also talked about the limitations of this method. But this is barely scratching the surface of what can be achieved using text mining. More advanced methods are available, and the amount you can learn using simpler techniques can often be extended if you apply them more creatively. 

```{r, echo=FALSE}
# PLAN
# Talk through code, especially TF-IDF and removing stop words etc. Show what table looks like as we manipulate it.
# Make a visualisation to show TF-IDF and talk about how the metric used influences what results we see. 
# Chronological visualisation shows the 1st third is about citizens rights, the middle is about nuclear energy, and the last 1/3rd is about parliaments role
# 
# STEMMING- should i stem words for tf-idf


# UNUSED/REMOVED
# 
# The scale at which you define what a document is depends on what you want to find out (this will make more sense in a bit).   
```
